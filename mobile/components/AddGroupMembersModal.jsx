...
const AddGroupMembersModal = ({
  ...
  const existingMemberIds = useMemo(() => {
    return existingMembers.map((member) => (typeof member === "object" ? member._id : member));
  }, [existingMembers]);
  ...
  const loadUsers = async () => {
    ...
    try {
      const response = await axios.post(`${REACT_APP_BACKEND_URL}/api/search-friend-user`, { search: "" });

      if (response.data && response.data.data) {
        // Filter out users who are already in the group
        const availableUsers = response.data.data.filter(
          (user) => !user.isGroup && !existingMemberIds.includes(user._id) && user._id !== currentUserId
        );
                ...
                <View className="items-center justify-center py-8">
                  {searchQuery.trim() ? (
                    ...
                    <>
                      ...
                      <Text className="text-center text-gray-500 mt-2">Không có người dùng nào có thể thêm</Text>
                    </>
                  )}
                </View>
              )}
